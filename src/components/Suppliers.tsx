...
import { Search, PlusCircle, Trash2, Edit3 } from "lucide-react";
...
export const Suppliers = () => {
  ...
  const handleAddSupplier = async () => {
    ...
    setShowAdd(false);
    setNewSupplier({ name: "", phone: "", gst_number: "", invoice_format: "" });
    fetchSuppliers();
  };
  ...
  const formFields = ["name", "phone", "gst_number", "invoice_format"];
    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-slate-400 dark:text-slate-500 font-medium mt-1 text-xs">
            Manage vendor relationships and track outstanding payables in INR.
          </p>
        </div>
        <Card className="px-6 py-4 bg-white dark:bg-slate-900 ring-1 ring-slate-100 dark:ring-slate-800 flex flex-col items-end">
          <span className="text-[9px] uppercase tracking-[0.2em] text-slate-300 dark:text-slate-600 font-bold mb-0.5">
            Total Outstanding
          </span>
          <span className="text-xl font-bold font-headline tabular-nums text-primary">
            {formatCurrency(
              suppliers.reduce((acc, s) => acc + s.current_balance, 0),
            )}
          </span>
        </Card>
      </header>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800">
                <th className="px-6 py-3 text-[9px] uppercase tracking-[0.15em] font-bold text-slate-400 dark:text-slate-500">
                  Supplier Details
                </th>
                <th className="px-6 py-3 text-[9px] uppercase tracking-[0.15em] font-bold text-slate-400 dark:text-slate-500">
                  Phone
                </th>
                <th className="px-6 py-3 text-[9px] uppercase tracking-[0.15em] font-bold text-slate-400 dark:text-slate-500 text-right">
                  Outstanding
                </th>
                <th className="px-6 py-3 text-[9px] uppercase tracking-[0.15em] font-bold text-slate-400 dark:text-slate-500 text-center">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {filteredSuppliers.length === 0 ? (
                <tr>
                  <td
                    colSpan={4}
                    className="px-6 py-8 text-center text-slate-300 dark:text-slate-700 text-xs italic font-medium"
                  >
                    No suppliers found
                  </td>
                </tr>
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-bold text-slate-900 dark:text-slate-100 text-sm font-headline">
                            {s.name}
                          </div>
                          <div className="text-[10px] text-slate-400 dark:text-slate-500 font-bold mt-0.5 tracking-tight">
                            {s.gst_number}
                          </div>
                        </div>
                      </div>
                    ...
                    <td className="px-6 py-4 text-right tabular-nums font-bold text-primary text-base">
                      {formatCurrency(s.current_balance)}
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex justify-center gap-1">
                        <Button
                          onClick={() => setShowPayment(s)}
                          className="px-3 py-1 text-[8px] uppercase tracking-widest"
                        >
                          Pay
                        </Button>
                        <button
            ...
            <div key={field} className="space-y-1">
              <div className="flex justify-between items-center">
                ...
                {field === "invoice_format" && (
                  <span className="text-[7px] text-primary/50 font-bold uppercase tracking-tight">
                    Use {"{{n}}"} for running number
                  </span>
                )}
              </div>
              <input
                ...
                placeholder={
                  field === "invoice_format" ? "e.g. UH/25-26/{{n}}" : ""
                }
              />
            </div>
          ))}
          <Button onClick={handleAddSupplier} className="w-full py-2 text-xs">
            Save Supplier
          </Button>
        </div>
      </Modal>
      ...
      {/* Payment Modal remains exactly the same */}
      <Modal
        isOpen={showPayment !== null}
        onClose={() => setShowPayment(null)}
        title="Record Payment"
      >
        <div className="space-y-6">
          <div className="p-4 bg-primary/5 rounded-2xl border border-primary/10">
            <p className="text-[9px] font-bold text-primary/50 uppercase tracking-widest mb-1">
              Paying To
            </p>
            ...
            <p className="text-[10px] text-primary font-bold mt-1">
              Outstanding: {formatCurrency(showPayment?.current_balance || 0)}
            </p>
          </div>
          <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {["Cash", "Cheque"].map((m) => (
                  <button
                    key={m}
                    onClick={() =>
                      setPaymentData({ ...paymentData, method: m as any })
                    }
                    className={cn(
            ...
            <Button onClick={handlePayment} className="w-full py-2.5 text-xs">
              Confirm Payment
            </Button>
          </div>
        </div>
